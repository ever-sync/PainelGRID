...
export class AuthService {
  ...
  async logout(refreshTokenRaw: string) {
    ...
    try {
      ...
      this.logger.warn(`Redis indisponivel ao encerrar sessao: ${(err as Error).message}`);
    }
  ...
  async resetPassword(dto: ResetPasswordDto) {
    ...
    const newPasswordHash = await bcrypt.hash(dto.new_password, BCRYPT_SALT_ROUNDS);
  ...
  async changePassword(authenticatedUser: AuthenticatedUser, dto: ChangePasswordDto) {
    ...
    const currentPasswordMatches = await bcrypt.compare(dto.current_password, user.password_hash);
  ...
  private async issueTokens(user: User, remember: boolean) {
    ...
    try {
      ...
      try {
        await this.redisService.client.del(this.getRefreshTokenKey(refreshTokenId));
      } catch {
  ...
  private async persistRefreshAllowlistOrThrow(
    ...
    try {
      const outcome = await this.redisService.client.set(key, userId, 'EX', ttlSeconds);
      if (outcome !== 'OK') {
  ...
  private async assertLoginAllowed(email: string) {
    ...
    if (Number.isFinite(attempts) && attempts >= AuthService.loginRateLimitMaxAttempts) {
  ...
  private shouldExposePasswordResetToken() {
    return (
      ...
      this.configService.get<string>('ALLOW_PASSWORD_RESET_TOKEN_RESPONSE') === 'true'
    );
  }
  ...
  private async revokeRefreshTokensForUser(userId: string) {
    ...
    try {
      ...
      do {
        ...
        const storedValues = await Promise.all(keys.map((key) => client.get(key)));
