...
describe('CoursesService', () => {
  ...
  describe('myProgress', () => {
    it('should throw if user is not vendor', async () => {
      await expect(service.myProgress(mockGestor)).rejects.toThrow(ForbiddenException);
    });

    it('should return vendor progress', async () => {
      jest
        .spyOn(prisma.courseProgress, 'findMany')
        .mockResolvedValue([{ id: '1', completed: true, course: {}, lesson: {} } as any]);
  ...
  describe('progressForVendor', () => {
    it('should throw if viewer is not gestor or client', async () => {
      await expect(service.progressForVendor(mockVendor, 'v1')).rejects.toThrow(ForbiddenException);
    });

    it('should throw if vendor is not found', async () => {
      jest.spyOn(prisma.user, 'findFirst').mockResolvedValue(null);
      await expect(service.progressForVendor(mockGestor, 'v1')).rejects.toThrow(NotFoundException);
    });

    it('should throw if gestor does not own the client', async () => {
      jest.spyOn(prisma.user, 'findFirst').mockResolvedValue({ id: 'v1', client_id: 'c1' } as any);
      jest.spyOn(prisma.client, 'findFirst').mockResolvedValue(null);
      await expect(service.progressForVendor(mockGestor, 'v1')).rejects.toThrow(ForbiddenException);
    });

    it('should return progress', async () => {
      jest.spyOn(prisma.user, 'findFirst').mockResolvedValue({ id: 'v1', client_id: 'c1' } as any);
      jest.spyOn(prisma.client, 'findFirst').mockResolvedValue({ id: 'c1' } as any);
      jest.spyOn(prisma.courseProgress, 'findMany').mockResolvedValue([{ id: '1' } as any]);
  ...
  describe('upsertLessonProgress', () => {
    it('should throw if user is not vendor', async () => {
      await expect(
        service.upsertLessonProgress(mockGestor, 'c1', 'l1', { completed: true }),
    ...
    it('should throw if lesson not found or course not published', async () => {
      ...
      await expect(
        service.upsertLessonProgress(mockVendor, 'c1', 'l1', { completed: true }),
    ...
    it('should upsert and return row', async () => {
      ...
      jest.spyOn(prisma.courseProgress, 'upsert').mockResolvedValue({ id: 'p1' } as any);

      const result = await service.upsertLessonProgress(mockVendor, 'c1', 'l1', {
        completed: true,
      });
