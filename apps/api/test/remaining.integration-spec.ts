...
describe("Remaining traceability endpoints (integration)", () => {
  ...
  async function createApp() {
    const mockClients = {
      create: jest
        .fn()
        .mockResolvedValue({ id: clientId, company_name: "Empresa Nova" }),
    };
    const mockCrm = {
    ...
    const mockCourses = {
      listPublished: jest
        .fn()
        .mockResolvedValue([{ id: "course-1", title: "Curso Demo" }]),
      progressForVendor: jest
        .fn()
        .mockResolvedValue([{ course_id: "course-1", progress: 100 }]),
    };
