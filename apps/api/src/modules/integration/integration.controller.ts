...
export class IntegrationController {
  ...
  @ApiQuery({
    name: "date_from",
    required: false,
    description: "ISO 8601 — eventos a partir desta data",
  })
  @ApiQuery({
    name: "date_to",
    required: false,
    description: "ISO 8601 — eventos até esta data",
  })
  @ApiQuery({
    name: "status",
    required: false,
    description: "Filtra por status do evento",
  })
  listEvents(
  ...
  @ApiQuery({
    ...
    required: false,
    description:
      "Filtra por status: pending | scheduled | confirmed | cancelled | checked_in",
  })
  @ApiQuery({
    name: "search",
    required: false,
    description:
      "Busca por nome, e-mail ou telefone (parcial, insensível a maiúsculas)",
  })
  @ApiQuery({
    name: "source",
    required: false,
  ...
  @ApiQuery({
    ...
    required: false,
    description:
      "ISO 8601 — retorna leads atualizados a partir desta data/hora",
  })
  @ApiQuery({
    name: "cursor",
    required: false,
  ...
  async listLeads(@Query() query: FindLeadsQueryDto) {
    // A sincronizacao do historico legado pertence aos endpoints de conversa.
    // Fazê-la durante a consulta de leads competia por conexoes com as chamadas
    // seguintes do Rubinho, mesmo quando iniciada em background.
  ...
  @ApiResponse({
    status: 201,
    description:
      "Lead criado ou encontrado. Campo `already_existed` indica se já existia.",
  })
  @ApiResponse({
    status: 400,
  ...
  @ApiResponse({
    status: 201,
    description:
      "Lote processado. Retorna quantos leads foram criados ou atualizados.",
  })
  @ApiResponse({
    status: 403,
    description:
      "Formulario Meta nao esta vinculado ao cliente desta credencial.",
  })
  createFacebookLeads(
    @Body(
      new ParseArrayPipe({
  ...
  @ApiResponse({
    status: 201,
    description: "Reconciliação concluída (ou simulada com dry_run)",
  })
  ...
  @ApiResponse({
    status: 400,
    description: "Telefone já cadastrado para outro lead",
  })
  @ApiResponse({ status: 404, description: "Lead não encontrado" })
  patchLead(
    @Param("id", new ParseUUIDPipe()) id: string,
    @Body() dto: IntegrationPatchLeadDto,
  ) {
  ...
  @ApiResponse({
    status: 404,
    description: "Lead, pipeline ou etapa não encontrados",
  })
  moveLead(
    @Param("id", new ParseUUIDPipe()) id: string,
  ...
  @ApiOperation({
    summary:
      "Move lead no CRM usando apenas o sufixo da etapa (para uso pela IA)",
    description:
  ...
  moveLeadBySuffix(
    ...
    return this.crmService.moveLeadBySuffix(
      id,
      dto.stage_suffix,
      dto.notes,
      dto.source,
  ...
  @ApiOperation({
    summary:
      "Move lead no CRM usando o sufixo no path da URL (uso simplificado pela IA)",
    description:
  ...
  @ApiOperation({
    summary:
      "Retorna a configuração ativa do Rubinho vinculada a um evento ou lead",
    description:
  ...
  @ApiResponse({
    status: 200,
    description: "Configuração do Rubinho retornada com sucesso",
  })
  ...
  @ApiQuery({
    name: "event_id",
    required: false,
    description: "UUID do evento para buscar o robô",
  })
  @ApiQuery({
    name: "lead_id",
    required: false,
    description: "UUID do lead para buscar o robô",
  })
  @ApiQuery({
