...
describe('EventDashboardService.getTvDashboard', () => {
  ...
  it('lança NotFound quando evento não existe', async () => {
    ...
    await expect(service.getTvDashboard(gestorUser, eventId)).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
  ...
  it('bloqueia cliente que não participa do evento', async () => {
    setupEvent();
    await expect(service.getTvDashboard(otherClienteUser, eventId)).rejects.toBeInstanceOf(
      ForbiddenException,
    );
  });
  ...
  it('agrega funil, vendedores, equipes, segmentos e diário', async () => {
    ...
    prisma.salesTeam.findMany.mockResolvedValue([
      {
        ...
        name: 'Ferrari',
        members: [
          { user_id: vendor1, user: { id: vendor1, name: 'Ana', client_id: clientA } },
          { user_id: vendor2, user: { id: vendor2, name: 'Bruno', client_id: clientA } },
        ],
      },
    ]);
    ...
    const bruno = result.vendors.find((vendor) => vendor.vendor_id === vendor2)!;
    ...
    expect(dia22).toMatchObject({ leads: 2, scheduled: 1, sold: 1, checked_in: 1 });
    const dia23 = result.daily.find((day) => day.date === '2026-05-23')!;
    expect(dia23).toMatchObject({ leads: 2, scheduled: 1, sold: 1, checked_in: 1 });
