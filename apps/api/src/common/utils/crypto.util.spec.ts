import { encryptCheckinToken, decryptCheckinToken, generateRawCheckinToken } from './crypto.util';
...
describe('CryptoUtil', () => {
  ...
  it('should return original value on decryption if length is incorrect', () => {
    ...
    expect(decryptCheckinToken('not-hex-at-all-should-be-twenty-four-chars', secret)).toBe(
      'not-hex-at-all-should-be-twenty-four-chars',
    );
  });
});
