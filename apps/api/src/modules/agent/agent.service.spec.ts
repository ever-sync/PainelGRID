...
describe('AgentService', () => {
  ...
  it('monta contexto consolidado a partir de conversation_id', async () => {
    ...
    const result = await service.getWhatsappContext({ conversation_id: conversationId });
  ...
  it('calcula disponibilidade por evento e bloqueios do lead', async () => {
    ...
    expect(result.events[0].blocking_reasons).toContain('lead_has_active_appointment');
