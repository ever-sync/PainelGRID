...
describe("PublicService", () => {
  ...
  describe("checkInPreview", () => {
    it("should rate limit requests", async () => {
      ...
      for (let i = 0; i < 48; i++) {
        await expect(
          service.checkInPreview("voucher", "ip1"),
        ).resolves.toBeDefined();
      }

      await expect(service.checkInPreview("voucher", "ip1")).rejects.toThrow(
        HttpException,
      );
    });
    ...
    it("should throw NotFoundException if voucher is invalid", async () => {
      ...
      await expect(service.checkInPreview("invalid", "ip2")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("should throw NotFoundException if lead is not found", async () => {
      ...
      await expect(service.checkInPreview("voucher", "ip3")).rejects.toThrow(
        NotFoundException,
      );
    });
