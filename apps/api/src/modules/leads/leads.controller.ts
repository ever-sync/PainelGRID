...
export class LeadsController {
  ...
  findAll(@Query() query: FindLeadsQueryDto, @CurrentUser() user: AuthenticatedUser) {
  ...
  @Get('lookup')
  @Roles(Role.GESTOR, Role.CLIENTE, Role.VENDEDOR, Role.RECEPCAO)
  @ApiOperation({ summary: 'Busca leads por CPF ou Telefone para auto-preenchimento' })
  ...
  @ApiResponse({ status: 403, description: 'Acesso permitido apenas para recepção, vendedor ou gestor' })
  @ApiResponse({ status: 404, description: 'Token inválido ou lead não encontrado' })
  checkInByToken(@Body() dto: CheckInByTokenDto, @CurrentUser() user: AuthenticatedUser) {
  ...
  @Roles(Role.VENDEDOR)
  @ApiOperation({ summary: 'Atribui lead sem vendedor ao vendedor autenticado' })
  @ApiResponse({ status: 200, description: 'Lead atribuído com sucesso' })
  @ApiResponse({ status: 400, description: 'Lead já possui vendedor' })
  @ApiResponse({ status: 403, description: 'Sem permissão para este lead' })
  @ApiResponse({ status: 404, description: 'Lead não encontrado' })
  assignToMe(@Param('id', new ParseUUIDPipe()) id: string, @CurrentUser() user: AuthenticatedUser) {
  ...
  @ApiOperation({ summary: 'Notifica o vendedor vinculado que o cliente chegou (TV)' })
  ...
  @ApiResponse({ status: 403, description: 'Sem permissão' })
  @ApiResponse({ status: 404, description: 'Lead não encontrado' })
  callVendor(@Param('id', new ParseUUIDPipe()) id: string, @CurrentUser() user: AuthenticatedUser) {
  ...
  @ApiResponse({ status: 400, description: 'Atualização inválida para o papel atual' })
  @ApiResponse({ status: 403, description: 'Sem permissão para atualizar este lead' })
  @ApiResponse({ status: 404, description: 'Lead não encontrado' })
  update(
    @Param('id', new ParseUUIDPipe()) id: string,
  ...
  @ApiResponse({ status: 403, description: 'Sem permissão para excluir este lead' })
  @ApiResponse({ status: 404, description: 'Lead não encontrado' })
