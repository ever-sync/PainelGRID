-- Normaliza a ordem usando valores temporarios unicos por pipeline.
WITH ranked AS (
  SELECT id,
         100000000 + row_number() OVER (PARTITION BY pipeline_id ORDER BY id)::int AS temp_order
  FROM crm_stages
)
UPDATE crm_stages AS s
SET display_order = ranked.temp_order
FROM ranked
