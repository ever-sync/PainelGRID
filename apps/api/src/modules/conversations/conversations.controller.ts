...
export class ConversationsController {
  ...
  @ApiOperation({
    summary: "Lista conversas filtradas pelo cliente do usuário",
  })
  @ApiResponse({ status: 200, description: "Conversas retornadas com sucesso" })
  @ApiResponse({
    status: 403,
    description: "Sem permissão para listar conversas",
  })
  findAll(
  ...
  @ApiResponse({
    status: 201,
    description: "Conversa existente ou criada com sucesso",
  })
  @ApiResponse({
    status: 403,
  ...
  @ApiResponse({
    status: 404,
    description: "Lead não encontrado para o cliente informado",
  })
  ensure(
  ...
  @Roles(Role.GESTOR, Role.CLIENTE, Role.VENDEDOR, Role.RECEPCAO)
  @ApiOperation({
    summary: "Entrega a midia de uma mensagem WhatsApp autenticada pela API",
  })
  async messageMedia(
    ...
    const media = await this.conversationsService.downloadMessageMedia(
      user,
      messageId,
    );
  ...
  @ApiResponse({
    status: 403,
    description: "Sem permissão para enviar nesta conversa",
  })
  addMessage(
    @Param("id", new ParseUUIDPipe()) id: string,
  ...
  @ApiOperation({
    summary: "Envia arquivo/foto/video em uma conversa WhatsApp",
  })
