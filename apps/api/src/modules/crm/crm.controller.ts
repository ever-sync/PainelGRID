...
export class CrmController {
  ...
  @Roles(Role.GESTOR, Role.CLIENTE, Role.VENDEDOR)
  @ApiOperation({
    summary: "Retorna histórico de movimentações do lead no CRM",
  })
  ...
  @Roles(Role.GESTOR, Role.CLIENTE, Role.VENDEDOR)
  @ApiOperation({
    summary: "Retorna a linha do tempo completa do lead (todos os eventos)",
  })
  ...
  @ApiResponse({
    status: 404,
    description: "Lead/pipeline/etapa não encontrado",
  })
  moveLead(
    @Param("leadId", new ParseUUIDPipe()) leadId: string,
    ...
    return this.crmService.moveLeadByCodes(
      leadId,
      dto,
      user.sub,
      idempotencyKey,
