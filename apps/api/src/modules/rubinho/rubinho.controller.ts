...
export class RubinhoController {
  ...
  create(@CurrentUser() user: AuthenticatedUser, @Body() dto: CreateRubinhoDto) {
  ...
  findAll(@CurrentUser() user: AuthenticatedUser, @Query('client_id') clientId: string) {
    return this.rubinhoService.findAll(user, clientId);
  }

  @Get(':id')
  @Roles(Role.GESTOR, Role.CLIENTE)
  @ApiOperation({ summary: 'Busca os detalhes de um agente Rubinho específico' })
  ...
  @ApiResponse({ status: 404, description: 'Agente não encontrado' })
  delete(@CurrentUser() user: AuthenticatedUser, @Param('id', new ParseUUIDPipe()) id: string) {
  ...
  @Post(':id/faqs')
  @Roles(Role.GESTOR, Role.CLIENTE)
  @ApiOperation({ summary: 'Adiciona uma pergunta/resposta frequente (FAQ) ao agente Rubinho' })
  ...
  @Roles(Role.GESTOR, Role.CLIENTE)
  @ApiOperation({
    summary: 'Adiciona um documento de apoio (base de conhecimento) ao agente Rubinho',
  })
