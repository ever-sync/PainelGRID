...
describe("Core modules read endpoints (integration)", () => {
  ...
  async function createApp() {
    const mockClients = {
      findAllForUser: jest
        .fn()
        .mockResolvedValue([{ id: clientId, company_name: "Empresa Demo" }]),
      findOneForUser: jest
        .fn()
        .mockResolvedValue({ id: clientId, company_name: "Empresa Demo" }),
    };
    const mockEvents = {
      findAll: jest
        .fn()
        .mockResolvedValue([{ id: "evt-1", name: "Evento Demo" }]),
    };
    ...
    const mockCampaigns = {
      findAll: jest
        .fn()
        .mockResolvedValue([{ id: "camp-1", name: "Campanha Demo" }]),
    };
    const mockConversations = {
      findAll: jest.fn().mockResolvedValue([{ id: convoId }]),
      findMessages: jest
        .fn()
        .mockResolvedValue([{ id: "msg-1", content: "Oi" }]),
      addMessage: jest
        .fn()
        .mockResolvedValue({ id: "msg-2", content: "Nova mensagem" }),
    };
    const mockLeads = {
      findAll: jest
        .fn()
        .mockResolvedValue([{ id: "lead-1", name: "Lead Demo" }]),
    };
