...
describe("SalesTeamsService", () => {
  ...
  it("permite adicionar vendedor de outro cliente ao time do evento", async () => {
    ...
    const result = await service.addMember(gestorUser, teamId, {
      user_id: vendorId,
    } as never);
  ...
  it("bloqueia acesso a evento de outro cliente", async () => {
    ...
    await expect(service.findAll(clienteUser, eventId)).rejects.toBeInstanceOf(
      ForbiddenException,
    );
  });
  ...
  it("lança NotFoundException quando o time não existe", async () => {
    ...
    await expect(service.remove(gestorUser, teamId)).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
