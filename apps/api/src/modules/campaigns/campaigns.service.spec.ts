...
describe('CampaignsService', () => {
  ...
  it('permite gestor com vínculo ao cliente e retorna campanhas mapeadas', async () => {
    ...
    prisma.campaign.findMany.mockResolvedValue([
      {
        ...
        vendors: [
          {
            vendor_id: 'vendor-1',
            vendor: { id: 'vendor-1', name: 'Vendedor 1', email: 'v1@teste.com' },
          },
        ],
      },
    ]);
    ...
    const result = await service.findAll(
      { sub: 'gestor-1', role: Role.GESTOR, email: 'g@teste.com', name: 'Gestor' },
      { client_id: clientId },
    );
  ...
  it('bloqueia perfil sem permissão', async () => {
    await expect(
      service.findAll(
        { sub: 'admin-1', role: 'admin' as Role, email: 'a@teste.com', name: 'Admin' },
        { client_id: clientId },
      ),
    ).rejects.toBeInstanceOf(ForbiddenException);
  });
