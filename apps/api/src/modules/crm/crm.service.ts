...
export class CrmService {
  ...
  async getDashboardReport(query: GetDashboardReportQueryDto) {
    ...
    const stageNameById = Object.fromEntries(
      stageNames.map((s) => [s.id, s.name]),
    );

    const stageDistribution = stageGroups.map((g) => ({
      stage: g.crm_stage_id
        ? (stageNameById[g.crm_stage_id] ?? "Sem etapa")
        : "Sem etapa",
      count: g._count.id,
    }));
    ...
    const campaignTotals = records.reduce<Record<string, number>>(
      (acc, record) => {
    ...
    const convertedCount =
      stageDistribution.find((s) => s.stage === "Convertido")?.count ?? 0;
  ...
  async findPipelineByCode(code: string) {
    ...
    if (!pipeline) {
      throw new NotFoundException(
        "Pipeline CRM nao encontrado para o codigo informado",
      );
    }
  ...
  async createPipeline(dto: CreatePipelineDto) {
    ...
    if (new Set(explicitCodes).size !== explicitCodes.length) {
      throw new BadRequestException(
        "Existem codigos de etapa duplicados no payload",
      );
    }
    ...
    const pipelineData: Prisma.CrmPipelineCreateInput = {
      code: dto.code
        ...
        : await this.ensurePipelineCodeAvailable(
            clientIdToPipelineCode(dto.client_id),
          ),
      name: dto.name.trim(),
  ...
  async findStageByCode(code: string) {
    ...
    if (!stage) {
      throw new NotFoundException(
        "Etapa CRM nao encontrada para o codigo informado",
      );
    }
  ...
  async getLeadTimeline(leadId: string, user: AuthenticatedUser) {
    ...
    return entries.map((entry) => ({
      ...
      origin: entry.origin,
      from_stage: entry.from_stage_id
        ? (stageById.get(entry.from_stage_id) ?? null)
        : null,
      to_stage: entry.to_stage_id
        ? (stageById.get(entry.to_stage_id) ?? null)
        : null,
  ...
  async bulkMoveLeads(dto: BulkMoveLeadsDto, movingUser: AuthenticatedUser) {
    ...
    const destinationUrl =
      leads[0]?.client.webhook_url_n8n || "internal://n8n-not-configured";
    ...
    for (const lead of leads) {
      ...
      const alreadyAtTarget =
        lead.crm_pipeline_id === targetPipeline.id &&
        lead.crm_stage_id === targetStage.id;
      ...
      if (
        lead.crm_stage &&
        targetStage.display_order < lead.crm_stage.display_order
      ) {
        this.logger.warn(
    ...
    const notFoundIds = dto.lead_ids.filter(
  ...
  async moveLeadBySuffix(
    ...
    const stageCode = clientIdToStageCode(
      lead.client_id,
      stageSuffix.toUpperCase().trim(),
    );

    this.logger.log(`Move por sufixo resolvido para a etapa ${stageCode}`);

    return this.moveLeadByIntegration(leadId, {
  ...
  async moveLeadByCodes(
    ...
    if (!changedBy) {
      throw new NotFoundException(
        "Usuario responsavel pela movimentacao nao encontrado",
      );
    }
    ...
    const shouldDispatchWebhook =
      !suppressWebhook && this.shouldDispatchExternalWebhook(source);
    const destinationUrl =
      lead.client.webhook_url_n8n || "internal://n8n-not-configured";
    const mappedStatus = resolveConfirmationStatusForStage(
      lead.client.settings,
      targetStage.id,
    );
    if (
      mappedStatus === ConfirmationStatus.scheduled ||
      mappedStatus === ConfirmationStatus.confirmed
    ) {
  ...
  private async assertUserCanAccessPipelineClient(
  ...
  private createMoveLeadRequestHash(
    ...
    return createHash("sha256")
      .update(
        JSON.stringify({ leadId, pipelineCode, stageCode, notes, source }),
      )
      .digest("hex");
  }
  ...
  private async ensurePipelineCodeAvailable(code: string) {
    const normalizedCode = this.normalizeCode(code);
    const exists = await this.prisma.crmPipeline.findUnique({
      where: { code: normalizedCode },
    });
  ...
  private async ensureStageCodeAvailable(code: string) {
    const normalizedCode = this.normalizeCode(code);
    const exists = await this.prisma.crmStage.findUnique({
      where: { code: normalizedCode },
    });
  ...
  private async generateUniqueStageCode() {
    ...
    throw new InternalServerErrorException(
      "Nao foi possivel gerar codigo unico de etapa",
    );
  }
  ...
  private async maybeAwardContactedScore(
    ...
    if (!this.CONTACTED_STAGE_NAMES.has((toStage.name ?? "").toLowerCase()))
      return;
    ...
    try {
      ...
      this.logger.warn(
        `Nao foi possivel registrar score contacted: ${(err as Error).message}`,
      );
    }
  }
  ...
  private handlePrismaConflict(error: unknown): never {
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === "P2002") {
        throw new ConflictException(
          "Conflito de unicidade ao salvar pipeline/etapa CRM",
        );
      }
      if (error.code === "P2025") {
        throw new NotFoundException(
          "Registro nao encontrado ao tentar salvar pipeline/etapa CRM",
        );
      }
    }
