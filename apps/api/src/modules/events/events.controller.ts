...
export class EventsController {
  ...
  @ApiResponse({ status: 403, description: 'Sem permissão para listar eventos' })
  findAll(@Query() query: FindEventsQueryDto, @CurrentUser() user: AuthenticatedUser) {
  ...
  @ApiOperation({
    summary: 'Retorna funil compacto de todos os eventos ativos acessiveis ao usuario',
    description:
  ...
  @ApiResponse({ status: 400, description: 'Dados inválidos' })
  @ApiResponse({ status: 403, description: 'Apenas gestor/cliente pode atualizar' })
  @ApiResponse({ status: 404, description: 'Evento não encontrado' })
  update(
    @Param('id', new ParseUUIDPipe()) id: string,
  ...
  @Delete(':id')
  @Roles(Role.GESTOR, Role.CLIENTE)
  @ApiOperation({ summary: 'Exclui evento e desvincula seus registros dependentes' })
