  ...
  beforeEach(() => {
    prisma = {
      ...
      sale: {
        findMany: jest
          .fn()
          .mockResolvedValue([{ lead_id: 'lead-1', value: new Prisma.Decimal('100000') }]),
      },
      metaLeadImport: {
        findMany: jest.fn().mockResolvedValue([
          {
      ...
      appointment: {
        findMany: jest
          .fn()
          .mockResolvedValue([{ lead_id: 'lead-1', status: AppointmentStatus.completed }]),
        count: jest.fn().mockResolvedValue(1),
      },
      metaCampaignAssignment: {
        findMany: jest.fn().mockResolvedValue([{ meta_campaign_id: 'campaign-1' }]),
      },
      metaDailyInsight: {
        findMany: jest.fn().mockResolvedValue([
          {
