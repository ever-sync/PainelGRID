...
describe('AppointmentsService', () => {
  ...
  it('ao criar agendamento por vendedor, move para PRE_AGENDAMENTO e nao altera confirmation_status', async () => {
    prisma.lead.findUnique.mockResolvedValue(lead);
    prisma.event.findUnique.mockResolvedValue(event);
    prisma.eventParticipant.findMany.mockResolvedValue([{ client_id: clientId }]);
    prisma.apiIdempotencyRequest.findUnique.mockResolvedValue(null);
    ...
    prisma.crmStage.findFirst.mockResolvedValue({ id: 'stage-pre-agendamento' });
      ...
      expect.objectContaining({
        where: expect.objectContaining({
          code: expect.objectContaining({
            in: expect.arrayContaining([expect.stringContaining('_PRE_AGENDAMENTO')]),
          }),
        }),
      }),
    );
  ...
  it('permite criar appointment para empresa convidada no evento compartilhado', async () => {
    ...
    expect(result).toEqual(expect.objectContaining({ lead_id: leadId, event_id: eventId }));
  });
  ...
  it('registra no-show e move lead para etapa NO_SHOW no CRM', async () => {
    ...
    const result = await service.noShow(appointmentId, { reason: 'Nao apareceu' });
  ...
  it('bloqueia no-show quando appointment ja esta cancelado', async () => {
    ...
    await expect(service.noShow(appointmentId, {})).rejects.toBeInstanceOf(BadRequestException);
  });
});
