...
async function main() {
  const gestorEmail = (process.env.SEED_GESTOR_EMAIL || "gestor@leadflow.com")
    .toLowerCase()
    .trim();
  ...
  await upsertUser(
    "cliente@leadflow.com",
    "Cliente Demo",
    Role.cliente,
    demoHash,
  );
  await upsertUser(
    "vendedor@leadflow.com",
    "Vendedor Demo",
    Role.vendedor,
    demoHash,
  );
  await upsertUser(
    "recepcao@leadflow.com",
    "Recepcao Demo",
    Role.recepcao,
    demoHash,
  );
  ...
  for (const client of allClients) {
    ...
    console.log(
      `[seed] Pipeline CRM padrao atualizado provisionado para cliente ${client.id}`,
    );
  }
...
async function seedDemoCrmEventsLeadsCourses(clientId: string) {
  const { pipeline_id: pipelineId } = await provisionDefaultCrmPipeline(
    prisma,
    clientId,
  );
  ...
  const baseUrl =
    process.env.SEED_PUBLIC_APP_URL?.replace(/\/+$/, "") ||
    "http://localhost:5173";
  console.log(
  ...
  if (!course) {
    ...
    const hasLesson = await prisma.lesson.count({
