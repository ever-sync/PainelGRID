...
describe("IntegrationCredentialsService", () => {
  ...
  it("armazena apenas o hash e retorna o segredo uma unica vez na criacao", async () => {
    prisma.integrationCredential.create.mockImplementation(
      async ({ data }) => ({
        id: "credential-1",
        ...data,
        created_at: now,
    ...
    const result = await service.create(gestorId, clientId, {
      name: "n8n principal",
    });
    const createData =
      prisma.integrationCredential.create.mock.calls[0][0].data;
